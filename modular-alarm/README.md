...
`ModularAlarm.exe`를 더블클릭하면 주소창 없는 앱 창으로 실행됩니다. UI 파일, 알람 음원, Windows 알림 연결 기능은 실행 파일 안에 포함되어 있어 실행 파일 하나만 복사해도 기본 기능을 사용할 수 있습니다.

## Windows 시작 시 자동 실행

`EnableModularAlarmAutostart.cmd`를 한 번 실행하면 Windows 로그인 시 `ModularAlarm.exe`가 자동으로 실행됩니다. 해제하려면 `DisableModularAlarmAutostart.cmd`를 실행합니다.
...
첫 번째 알람 `훈련생 시간표`는 평일 기준으로 09:50, 10:50, 11:50, 12:50, 14:50, 15:50, 16:50, 17:50에 울리도록 기본 설정되어 있습니다.

알람 소리는 `alarm-sound.m4a`를 사용합니다. 처음 실행하면 하단의 `Enable sound & notifications` 버튼을 한 번 눌러 소리 재생을 허용해 주세요. 실행 파일에서는 Windows 알림 연결도 함께 준비됩니다.
