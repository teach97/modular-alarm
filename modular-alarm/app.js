...
function notify(alarm,module){const title=`${localizedName(alarm)} · ${localizedName(module)}`;const body=`${formatTime(module.time).value} ${formatTime(module.time).period}`;if(nativeRequest('/notify',{title,body}))return true;const permission=notificationPermission();if(permission==='unsupported')return false;if(permission!=='granted'){if(permission==='denied')toast(t('notificationBlocked'));return false;}try{const n=new Notification(title,{body,tag:`modular-${alarm.id}-${module.id}`,renotify:true,requireInteraction:true});n.onclick=()=>window.focus();return true;}catch(error){return false;}}
...
function stopRinging(){stopSound();ringingAlarm=null;$('#ring-backdrop').classList.add('hidden');document.title=defaultDocumentTitle;}
...
$('#alarm-form').addEventListener('submit',saveAlarm);$('#module-form').addEventListener('submit',saveModule);$('#enable-sound-button').addEventListener('click',()=>unlockAudio(true));$('#stop-alarm-button').addEventListener('click',stopRinging);document.addEventListener('pointerdown',()=>{if(!audioReady)unlockAudio(false)},{once:true});document.addEventListener('keydown',event=>{if(event.key==='Escape'){if(ringingAlarm)stopRinging();else closeModals()}});document.addEventListener('visibilitychange',checkAlarms);if(nativeNotifyPort)window.addEventListener('pagehide',nativeShutdown,{once:true});window.setInterval(checkAlarms,1000);renderAll();checkAlarms();
